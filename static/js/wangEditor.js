...
var config = {
    ...
    // 默认菜单配置
    menus: ['head', 'bold', 'fontSize', 'fontName', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo','allScreen'],
...
Panel.prototype = {
    ...
    show: function show() {
        ...
        var width = opt.width || 80; // 默认 300px
        var height = opt.height || 70; // 默认 300px
        if(opt.hasOwnProperty('css')){
          for(var i in opt.css){
            $container.css(i,opt.css[i]);
          }
        }
        ...
        // var containerH = $container[0]
        if (height) {
            $tabContentContainer.css('height', 100 + '%').css('overflow-y', 'auto').css('flex',1);
        }
...
Link.prototype = {
    ...
    _createPanel: function _createPanel(text, link) {
        ...
        var h = 90;
        var w = 80;
        if($('#editor-main').attr('class')=='all-screen' )
        {
          h=20;
          w=80;
        }else{
...
AllScreen.prototype = {
  ...
  onClick: function onClick(e) {
    ...
    //editor.cmd.do('redo');
    if(this._active){
      $('#editor-main').addClass('all-screen');
    }
    else{
      $('#editor-main').removeClass('all-screen');
    }
  }
};
...
Code.prototype = {
    ...
    _createPanel: function _createPanel(value) {
        ...
        var btnId = getRandom('btn');
          ...
          // if($('#editor-main').hasClass('all-screen')){
          //   console.log('全屏模式');
          // }
        var h = 90;
        var w = 80;
        ...
        var panel = new Panel(this, {
            ...
            tabs: [{
                ...
                title: '插入代码',
                // 模板
                tpl: '<div style=" height: 100%; display: flex ;flex-direction: column; ">\n                        <textarea id="' + textId + '" style="flex:1">' + value + '</textarea>\n                        <div class="w-e-button-container" style="padding:5px 0">\n                            <button id="' + btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    <div>',
                // 事件绑定
                events: [
                // 插入代码
                {
...
Emoticon.prototype = {
    ...
    _createPanel: function _createPanel() {
        ...
        // 记录属性
        this.panel = panel;
        var contentH = document.getElementsByClassName('w-e-panel-container')[0].offsetHeight-2;
        $('.w-e-text-container').css('padding-bottom','5px');
        $('.w-e-emoticon-container').css('display','flex').css('flex-wrap','wrap').css('justify-content','space-around');
        document.getElementsByClassName('w-e-panel-tab-content')[0].style.height = (contentH - 28)+'px';
        document.getElementsByClassName('w-e-panel-tab-content')[0].style.padding='10px 15px';
    },
...
MenuConstructors.allScreen = AllScreen;
...
Editor.prototype = {
    ...
    _initDom: function _initDom() {
        ...
        if (textSelector == null) {
            ...
            // 自行创建的，需要配置默认的样式
            $toolbarElem.css('background-color', '#fff').css('border-bottom', '1px solid #ddd');
            $textContainerElem.css('border-top', 'none').css('flex', '1' ).css('overflow','hidden');
        } else {
